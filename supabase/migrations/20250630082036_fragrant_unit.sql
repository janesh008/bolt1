/*
  # Create Permissions Table and Fix Roles Table
  
  1. New Tables
    - `permissions` - Stores available permissions for role-based access control
  ...
  3. Data
    - Insert standard permission records
    - Fix roles table permissions column type
*/
...
BEGIN
  ...
  LOOP
    ...
    IF jsonb_typeof(r.permissions) = 'array' THEN
      ...
      UPDATE roles 
      SET permissions = jsonb_build_array(SELECT jsonb_build_array(unnest(perm_array)))
      WHERE id = r.id;
    END IF;
  END LOOP;
END;
